...
import injectCleanseIdentityMap from "../../../../../src/components/Identity/injectCleanseIdentityMap";

describe("Identity:injectCleanseIdentityMap", () => {
  let logger;
  let cleanseIdentityMap;

  beforeEach(() => {
    logger = jasmine.createSpyObj("logger", ["info"]);
    cleanseIdentityMap = injectCleanseIdentityMap({
      logger
    });
  });
  ...
  it("returns identity map after removing identifiers without IDs and namespaces without identifiers", () => {
    ...
    const result = cleanseIdentityMap(input);
    ...
    expect(logger.info.calls.allArgs()).toEqual(expectedLogArgsByCallIndex);
  });
});
